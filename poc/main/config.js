...
const DEFAULTS = {
  ...
  // POC-only: when true, every RX packet is logged to diagnostics.
  // Default off so the ring buffer isn't drowned by 50 Hz telemetry.
  verboseRxLogging: false,
  // Renderer-only display option (mirrored here so save/load round-trips it).
...
function getConfig() {
  ...
  if (!_loadLogged) {
    _loadLogged = true;
    try {
      diag.info('config', 'Loaded settings', { path: store.path });
    } catch {}
  }
  return cfg;
}
...
function saveConfig(cfg) {
  ...
  if (changed.length) {
    try { diag.info('config', `Saved settings (${changed.length} key${changed.length === 1 ? '' : 's'})`, { changed }); } catch {}
  }
}
